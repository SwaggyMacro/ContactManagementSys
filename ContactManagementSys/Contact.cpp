...
void ContactList::sortByName() {
	...
	do {
		...
		while (current->next != nextNode) {
			...
			QString qstrName = QString::fromStdString(current->name);
			wchar_t* wstrName = new wchar_t[qstrName.length() + 1];
			wstrName[qstrName.length()] = '\0';
			qstrName.toWCharArray(wstrName);

			// wchar_t to std::string
			std::string firstLetter;
			std::string nextFirstLetter;
			if (Pinyin::IsChinese(*wstrName)) {
				firstLetter = Pinyin::GetPinyins(*wstrName)[0].substr(0, 1);
			}
			else {
				firstLetter = current->name.substr(0, 1);
			}
			if (current->next != nullptr) {
				QString qstrNextName = QString::fromStdString(current->next->name);
				wchar_t* wstrNextName = new wchar_t[qstrNextName.length() + 1];
				wstrName[qstrName.length()] = '\0';
				qstrNextName.toWCharArray(wstrNextName);
				if (Pinyin::IsChinese(*wstrNextName)) {
					nextFirstLetter = Pinyin::GetPinyins(*wstrNextName)[0].substr(0, 1);
				}
				else {
					nextFirstLetter = current->next->name.substr(0, 1);
				}
			}
			if (QString::fromStdString(firstLetter).toUpper().toStdString() > QString::fromStdString(nextFirstLetter).toUpper().toStdString()) {
